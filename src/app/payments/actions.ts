...
export async function createRazorpaySubscription(
  ...
  if (!userData) {
    console.error(
      "[Payment Action] Authentication failed, could not get user data."
    );
  ...
  try {
    ...
    console.log(
      `[Payment Action] Successfully created and stored Razorpay subscription ${subscription.id} for user ${userData.uid}.`
    );
    return {
...
export async function syncRazorpaySubscription(
  ...
  console.log(
    `[syncRazorpay] Starting subscription sync for user: ${uid} (email: ${email})`
  );

  try {
    const userProfileRef = db.ref(`user_profiles/${uid}`);
    const subIdSnapshot = await userProfileRef
    ...
    if (!subscriptionId) {
      console.log(
        `[syncRazorpay] No subscription ID found in DB for user ${uid}.`
      );
      return {
        success: false,
        error: "Could not find a subscription reference for your account.",
      };
    }

    console.log(
      `[syncRazorpay] Found subscription ID ${subscriptionId} for user ${uid}. Fetching from Razorpay.`
    );
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      console.log(
        `[syncRazorpay] Attempt ${i + 1}: Status is '${tempSubDetails.status}'.`
      );
    ...
    if (
      subscriptionDetails &&
      ["active", "completed"].includes(subscriptionDetails.status)
    ) {
      console.log(
        `[syncRazorpay] Found valid subscription ${subscriptionDetails.id} (Plan: ${subscriptionDetails.plan_id}, Status: ${subscriptionDetails.status}) for user ${uid}.`
      );
      ...
      console.log(
        `[syncRazorpay] Subscription ${subscriptionId} for user ${uid} is not active or completed after retries. Final status: ${subscriptionDetails?.status}.`
      );
      return {
        success: false,
        error:
          "We could not confirm an active Pro subscription. Please try restoring your purchase in Settings or contact support.",
      };
    }
  } catch (error) {
    console.error(
      `Error during manual subscription sync for user ${uid}:`,
      error
    );
    return {
      success: false,
      error:
        "An unexpected error occurred while checking your subscription status.",
    };
  }
}
...
export async function cancelRazorpaySubscription(): Promise<{
  ...
  try {
    ...
    if (!subData || !subData.id) {
      console.warn(
        `[CancelSub] No active subscription found in DB for user ${user.uid} to cancel.`
      );
      return {
        success: false,
    ...
    console.log(
      `[CancelSub] Found subscription ${subData.id} for user ${user.uid}. Current DB Status: ${subData.status}`
    );
    ...
    const finalSubData = (
      await userProfileRef.child("subscription").once("value")
    ).val();
    return { success: true, subscription: finalSubData };
  } catch (error) {
    console.error(
      `[CancelSub] Failed to cancel subscription for user ${user.uid}:`,
      error
    );
    const message =
