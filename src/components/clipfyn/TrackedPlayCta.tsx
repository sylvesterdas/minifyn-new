...
export function TrackedPlayCta({
  placement,
  href = "https://play.google.com/store/apps/details?id=com.minifyn.clipfyn",
  ariaLabel = "Get ClipFyn on Google Play",
        ...
        for (const entry of entries) {
          if (entry.isIntersecting && entry.intersectionRatio >= 0.5 && !impressionFired.current) {
            ...
            if (typeof window !== "undefined" && (window as any).gtag) {
              (window as any).gtag("event", "clipfyn_cta_impression", {
                placement,
  ...
  function handleClick() {
    if (typeof window !== "undefined" && (window as any).gtag) {
      (window as any).gtag("event", "clipfyn_download_click", {
        placement,
        page_location: window.location.href
      });
    }
