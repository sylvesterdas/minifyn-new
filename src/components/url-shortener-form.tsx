    ...
    useEffect(() => {
        if (!state.success && state.message) {
            if (state.errorCode === 'ANON_LIMIT_REACHED') {
                 ...
                 trackEvent({ action: 'conversion_prompt_shown', category: 'url_shortener', label: 'anonymous_rate_limit' });
            } else {
                 toast({
                    title: "Oops!",
                ...
                trackEvent({ action: 'shorten_url_error', category: 'url_shortener', label: state.message });
            }
        } else if (state.success && state.shortUrl) {
            setShortenedUrl(state.shortUrl);
            setShowSignupPrompt(false); // Reset prompt on success
            formRef.current?.reset();
            trackEvent({ action: 'shorten_url_success', category: 'url_shortener', label: state.shortUrl });
        }
    }, [state, toast]);
    ...
    const handleCopy = () => {
        if (shortenedUrl) {
            navigator.clipboard.writeText(shortenedUrl).then(() => {
                ...
                trackEvent({ action: 'copy_short_url', category: 'url_shortener', label: shortenedUrl });
            });
        }
    };
    
    const handleSignUpClick = () => {
        trackEvent({ action: 'click_signup_prompt', category: 'url_shortener', label: 'anonymous_rate_limit_cta' });
    }
                ...
                <CardFooter className="flex flex-col gap-4">
                     {showSignupPrompt ? (
                        <div className="w-full text-center space-y-3 animate-in fade-in duration-500">
                             ...
                             <Button asChild className="w-full font-semibold" onClick={handleSignUpClick}>
